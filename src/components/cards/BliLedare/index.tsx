...
const BliLedareHome = () => {
 return (
  ...
  // <article className="bg-[url(/images/Backgrounds/darkBlueWave.png)]">
  <article className="bg-primary font-body text-background p-6 md:grid md:place-items-center">
    <div className="md:grid md:place-items-center w-full">
      ...
      <p className="text-muted text-base md:text-xl pt-4 pb-4">
        ...
        Vi skickar dig på Scouternas ledarutbildning och du får stöd av erfarna ledare från dag ett.
      </p>
      <div className="flex flex-col md:flex-row">
          ...
          {bliLedareCardContent && bliLedareCardContent.map((card, index) => (
            <li key={index} className="flex border border-muted rounded-md pt-4 pb-4 bg-[#1A496F]">
              ...
              <div>
                ...
                <p className="text-sm text-balance md:text-wrap">{card.subtitle}</p>
              </div>
            </li>
          ))}
        </ul>
        ...
        <div className="flex flex-col justify-between items-center text-center gap-6 font-bold md:pl-8">
          ...
          <a className="w-full border border-accent bg-primary-hover rounded-sm p-4 text-lg cursor-pointer">Anmäll ditt intresse <span className="pl-1">&#8594;</span></a>
          <a className="border border-accent rounded-sm p-4 w-[70%] text-base text-accent cursor-pointer">Mer information här</a>
        </div>
      </div>
    </div>
  </article>
